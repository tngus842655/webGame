...
- 콘텐츠 등급 설문
- 타겟층 및 콘텐츠 — **연령대를 13세 미만이 포함되게 고르면 안 된다.** `bubblewrap init`도
  경고하듯 TWA는 Play for Families 정책과 호환되지 않는다. 미니게임 모음이라 아동층을
  넣고 싶어지지만, 넣는 순간 TWA로는 심사를 통과할 수 없다. **13세 이상**으로 간다.
...
스크린샷은 폰에서 게임 몇 개를 실행해 찍으면 된다.

아이콘은 영문판(`icon-eng-*`)을 쓴다. 한글판(`icon-192-v1.png`, `icon-512-v1.png`)은
이전 버전이라 매니페스트에서 참조하지 않는다.
...
여기서 9단계(assetlinks)까지 마쳐 주소창이 사라지는 것까지 확인한다.

### 8-2. 비공개 테스트로 승급

**테스트 및 출시 → 테스트 → 비공개 테스트** → 같은 AAB를 승급하거나 새로 올린다.
여기부터 10단계의 12명·14일이 적용된다.

> **빌드가 완벽해질 때까지 기다릴 필요는 없다.** 새 빌드를 올려도 14일 카운터는
> 리셋되지 않는다. 시계는 특정 빌드가 아니라 테스터 12명이 옵트인을 유지하는
> 연속 일수를 센다. 심사도 미리 끝내 두는 게 이득이라, 승급은 빠를수록 좋다.
...
| 지문 | 출처 | 확인된 것 |
| --- | --- | --- |
| `4C:B0:2A...` | 앱 서명 → 앱 서명 키 인증서 | **내부 앱 공유로 설치했을 때 이 지문으로 검증됐다** |
| `26:59:36...` | 앱 서명 → 업로드 키 인증서 | 쓰이는 경로를 아직 확인하지 못했다 |

**내부 앱 공유 페이지의 "내부 테스트 인증서"(`59:B5...`)는 넣지 않아도 된다.**
그 지문 없이도 내부 앱 공유 설치에서 주소창이 사라지는 것을 확인했다.
구글이 앱 서명 키로 재서명해서 배포하기 때문으로 보인다.

업로드 키 지문은 이론상 로컬 빌드 APK를 직접 설치할 때만 쓰인다. 다만 지금
조합(`4C` + `26`)이 실제로 동작하는 것을 확인한 상태라, **검증 없이 빼지 않는다.**
줄이려면 하나만 남기고 배포한 뒤 앱을 지웠다 다시 깔아 확인할 것.
...
리셋의 원인은 빌드가 아니라 **테스터 구성 변화**다.

- 인원이 **12명 아래로 떨어지면** 연속 일수가 깨진다
- **중간에 테스터를 새로 추가하면** 사이클이 다시 시작될 수 있다

그래서 **계정을 한 명씩 늘려가는 게 제일 위험하다.** 12명을 다 모을 때까지 기다렸다가
한 번에 등록한다. 중간 이탈을 감안해 14~15명을 확보해 두는 편이 안전하다.

반대로 **빌드 업데이트는 안전하다.** 테스트 기간 중 버그를 고쳐 새 버전을 올리는 건
카운터에 영향이 없고, 오히려 정상적인 진행이다.

가족·지인 계정을 미리 확보해 두는 게 현실적으로 제일 빠르다.
