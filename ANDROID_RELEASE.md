...
**설치 경로마다 서명하는 키가 다르다. 실제로 쓰는 경로만 넣는다.**

| 설치 경로 | 서명 키 | 지문 위치 |
| --- | --- | --- |
| Play 스토어 | 앱 서명 키 (구글이 재서명) | 앱 서명 → 앱 서명 키 인증서 |
| 내부 앱 공유 | 내부 테스트 인증서 | 내부 앱 공유 → 내부 테스트 인증서 |
| APK 직접 설치 | 업로드 키 (내 키스토어) | 앱 서명 → 업로드 키 인증서 |

지금 넣어 둔 건 앞의 둘이다. **업로드 키는 뺐다** — AAB만 만들어 올리고 APK를
폰에 직접 꽂는 일이 없어서, 쓰지 않는 경로였다. 나중에 로컬 APK로 테스트할
일이 생기면 그때 추가한다.

**내부 앱 공유 지문도 정식 출시 전에 뺀다.** 스토어 등록정보가 없어도 앱을
폰에 깔아 볼 수 있어서 개발 중에만 쓰는 경로고, 프로덕션에는 앱 서명 키
하나만 있으면 된다.
