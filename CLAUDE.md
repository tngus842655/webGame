...
## 6. AI가 아닌 사람의 감성으로
문구나 시각적 표현, UI를 구현하는 데 있어 업계 최고의 전문가 관점에서 판단하고 개발할 것.
AI 작성한 듯한 문구나 이미지 아이콘은 사용을 최소화하고 사람이 직접 구현한 것처럼 개발할 것.
구현 중 이미지나 아이콘 필요한 경우는 사용자에게 요청할 것 (요청 시 구체적이고 명확하게, 참조할 만한 사이트 URL 제시)