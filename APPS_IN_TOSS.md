...
| 증상 | 원인 | 고친 곳 |
| --- | --- | --- |
| 게임 중 더블탭하면 화면이 두드린 쪽으로 밀리고 반대쪽이 잘림 | 브라우저 기본 더블탭 확대. 캔버스는 `stage.ts`에서 이미 `touch-action:none`이라 범인은 점수바·하단바 같은 **바깥 HTML**이었다 | `main.css` body에 `touch-action: manipulation` |
| 게임 카드를 길게 누르면 링크 주소가 뜸 | 카드가 `RouterLink`(`<a>`)다. body의 `-webkit-touch-callout`은 **iOS 전용**이라 안드로이드 웹뷰에서는 안 먹는다 | `main.ts`에서 `contextmenu` 기본동작 차단 (입력칸은 예외) |

> ⚠️ **아직 실기기 검증 전이다.** 타입체크와 코드 리뷰만 거쳤다.
> 재확인할 때는 반드시 **수정 커밋 이후에 빌드한 번들**로 할 것 —
> 첫 번들 `20260730-1`은 수정 전 코드다.
> 그래도 재현되면 `touch-action`을 `body` 대신 `*`에 거는 방법이 남아 있다
> (iOS가 조상 요소의 `touch-action`을 무시하는 케이스가 알려져 있다).
