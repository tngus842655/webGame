...
1인 100원. 5,000포인트 한도의 2%다. 30만 원이면 전원이 완주해도 3,000명이고, 실제로는
중간 이탈이 있어 더 많이 들어온다.

**문구를 결과로 쓰지 말 것.** "3판 **하기**"는 되고 "3판 **이기기**"·"클리어"는 즉시 반려다.
체크리스트의 "시간이나 노동을 많이 요구하지 않나요?"는 30초짜리 게임 4판이라 방어할 수
있지만, 판단은 검수원에게 있다.
...
- [x] `getTossAnonymousKey()` — `src/shared/toss.ts`
- [x] `toss_anon_keys` 표와 `record_toss_anon_key()` — 게임 화면에 들어갈 때 기록한다.
      계정이 만들어지는 자리가 거기라서다
- [ ] **실기기 확인** — 샌드박스(`dev:toss`)는 mock을 주므로 QR로 봐야 한다.
      앱 데이터를 지우고 다시 들어갔을 때 `anon_key`가 그대로면(= 행이 늘지 않고 `user_id`와
      `updated_at`만 바뀌면) 이 설계가 성립한다. **여기서 값이 매번 달라지면 프로모션 전체를
      다시 설계해야 한다**
- [ ] 지급 이력 표 + 서버 판정. 지금 `record_toss_anon_key()`는 클라이언트가 보낸 값을 그대로
      믿는다. 지급을 걸 때는 서버가 `anon-key/verify`(mTLS)로 검증한 뒤 기록해야 한다
- [ ] 콘솔 프로모션 등록 → 검수 → `TEST_` 코드로 지급 테스트
