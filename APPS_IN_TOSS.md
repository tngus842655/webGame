...
있다. 창이 짧고 콘솔의 '1인 하루 최대 100원'이 백스톱이라 이 순서를 택했다 — 반대로 하면
토스앱 버전이 낮아 지급이 실패한 사람이 업데이트한 뒤에도 영영 못 받는다.
...
- [ ] **실기기 확인** — 샌드박스(`dev:toss`)는 mock을 주므로 QR로 봐야 한다.
      ...
      다시 설계해야 한다**
- [ ] **고지 UI.** 지급 시점·조건·제한과 "본 프로모션은 사전 고지 없이 중단될 수 있습니다"를
      보여줘야 한다. 정책 필수 항목이라 **시작 전에 붙여야 한다** (테스트에는 필요 없다)
