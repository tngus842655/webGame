...
| 앱인토스 웹뷰 | 안 남 — 1차 관찰 때는 났으므로 2차 수정으로 잡힌 것으로 본다 |
| iOS '홈 화면에 추가' | **재현** |

**iOS standalone에서만 남는다.** 주 배포처인 앱인토스와 Play(TWA = 크롬 엔진)는
영향이 없어서, 원인을 짚기 전까지 **의도적으로 두기로 했다.**

다시 볼 조건: iOS 홈화면 이용자가 늘거나, 이 증상으로 이탈한다는 신호가 잡힐 때.
...
| 게임 카드 롱프레스 시 주소 노출 | 검증 대기 |
| 더블탭 시 화면 밀림 | ⏸ iOS 홈화면에서만 재현 — 의도적으로 보류 |
