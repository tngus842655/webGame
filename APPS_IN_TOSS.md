...
토스 포인트를 지급하는 이벤트. 앱이 검색에 노출되기 시작하면 붙일 예정이라, 그때 쓸
결론만 남겨 둔다. 원문은 `toss-docs/promotion.md`.

**이 앱은 게임이라 쓸 수 있는 폭이 좁다.** 게임 결과 기반 보상(점수·승패·등수)과 확률형·
랜덤(룰렛·뽑기)이 불가다 — 게임 결과와 현금성 보상을 직접 잇는 것이 사행성 유도로 금지되어
있어서다. 게임 결과와 **무관한** 조건만 쓸 수 있다: 가입, 로그인, 튜토리얼 완료.
...
**대신 유입에 쓴다.** 프로모션은 광고 수익을 나눠주는 장치가 아니라 사용자를 사오는
장치다. 이 앱에 가장 맞는 건 **"토스로 로그인하기"** — 허용 유형에 명시된 회원가입·접속
보상이고, `userKey`로 1인 1회가 자연스럽게 묶여 어뷰징이 어렵고, 익명 계정에 계정 복구
경로가 생겨 기록을 붙잡는 실익까지 있다. "게임 3판 하기"도 결과 기반이 아니라 가능해
보이지만, "시간·노동을 많이 요구할 수 없다"는 조항의 판단이 검수원에게 달렸다.

**미리 알아 둘 함정**

- **혜택 탭 노출 여부는 나중에 못 바꾼다.** 미노출로 등록하면 새 프로모션을 만들어야 한다 —
  유입이 목적이면 처음부터 노출로 등록할 것
- **게임 미니앱은 서버 지급(S2S)을 못 쓰는 것으로 읽힌다** ("게임이 아닌 미니앱은 서버로
  지급하는 방식도 쓸 수 있어요"). SDK에 `grantPromotionRewardForGame`이 따로 있는 것도 그
  근거다. 클라이언트가 지급을 요청하는 구조라 **콘솔의 '1인 하루 최대 지급 금액'을 반드시
  걸 것**
- 미션 이름은 `~하기`로 끝나야 한다. 이동 URL은 `intoss://minigame30/ScreenName` 형식이라
  라우터가 그 스킴을 받는지 확인이 필요하다
- 1인 5,000포인트 이하. 사전 검수 영업일 2~3일, 포인트 지급 API 테스트 1회 필수
- `TEST_{promotionCode}`를 쓰면 예산을 안 쓰고 검증할 수 있다
- 문서 안에서 비즈 월렛 최소 충전액이 어긋난다 (앞쪽 "최소 30만 원", 3번 항목 "최소 50,000원")

**선행 조건: 토스 로그인이 실기기에서 검증되어야 한다.** 위 '로그인' 절의 미확인 항목이
그대로 남아 있다.
