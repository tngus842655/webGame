...
브라우저 기본 **더블탭 확대**다. 게임 캔버스는 `stage.ts`에서 이미 `touch-action:none`이라
범인은 점수바·하단바 같은 **바깥 HTML**이었다.

| 시도 | 결과 |
| --- | --- |
| `body`에 `touch-action: manipulation` | ❌ 그대로 재현. `touch-action`이 조상에서 자손으로 항상 먹지는 않는다 |
| `*`에 `touch-action: manipulation` + viewport에 `user-scalable=no` | 검증 대기 |

`user-scalable=no`는 일반 모바일 사파리에서는 접근성 정책으로 무시되지만,
**웹뷰와 '홈 화면에 추가'로 띄운 창에서는 먹는다.** 문제가 나던 곳이 정확히 그 둘이다.
대신 그 두 곳에서 핀치 확대가 막힌다 — 게임이라 확대할 일이 없다고 보고 받아들였다.
...
> ⚠️ 위 세 건 모두 **실기기 검증 전이다.** 타입체크와 빌드만 통과했다.
> 확인할 때는 반드시 **수정 커밋 이후에 빌드한 번들**로 할 것.
