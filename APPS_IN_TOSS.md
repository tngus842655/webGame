...
**끝난 것은 서버가 알고 화면이 따라간다.** 예산이 소진되거나 종료일이 지나도 앱은 그걸
알 길이 없어서, 홈 배너는 계속 '80원 더 받을 수 있어요'를, 안내 화면은 '조건을 채우면 바로
지급됩니다'를 띄웠다. 시키는 대로 게임을 해도 아무것도 들어오지 않는다. 지금은
`record_promotion_failure()`가 종료를 뜻하는 코드(`4112` 예산 부족 · `4105` 종료 ·
`4109` 실행중 아님)를 받으면 `promotion_state`를 끈다. 한 사람이 예산 부족을 받았다는 것은
나머지 전원도 못 받는다는 뜻이라, 처음 부딪힌 사람이 모두를 위해 끄는 구조다. 콘솔 종료일을
`ends_at`에 넣어두면 아무도 지급을 시도하지 않아도 날짜로 꺼진다.

**끄는 것은 자동, 되살리는 것은 사람이 한다.** `4112`(예산 부족)와 `4109`(일시정지)는
충전하거나 재개하면 풀리는데, 코드 한 번으로 영구 종료를 못 박으면 되살아난 뒤에도 꺼진
채로 남는다. 다시 켜려면 이 한 줄이다.
...
받은 포인트는 토스 혜택 탭에서 볼 수 있어요'로 바뀐다. 끝난 뒤에 처음 들어와 받은 것이
하나도 없는 사람에게만 배너를 아예 안 그린다. 고지 문구 자체는 상태에 따라 바꾸지 않는다 —
