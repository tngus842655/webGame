...
void loadLocale(locale.value).then(() => {
  ...
  void checkAppUpdate()
  // 미니앱의 프로모션 내역을 읽어 온다. 지급은 끝났고(프로모션 종료), 이제 남은 일은
  // 받은 사람에게 '얼마 받았고 이제 끝났다'를 보여주는 것뿐이다 — 계정을 새로 만들지는
  // 않으므로 위의 원칙(열어만 보고 나간 방문자는 계정을 만들지 않는다)이 그대로 지켜진다.
  // 웹·안드로이드 빌드에서는 상수가 false라 이 줄이 통째로 빠진다.
