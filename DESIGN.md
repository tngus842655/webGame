...
`popularityRanks(games)`에는 **반드시 현재 레지스트리의 게임만 넘긴다.** 인기도
캐시(`get_game_popularity`)는 `play_sessions`를 집계하는데, 이미 지운 게임의 기록도
그 표에 남아 있다. 그것까지 세면 화면에 없는 게임이 번호를 차지해 홈 목록에
6번·13번처럼 빈 자리가 생긴다.

**홈과 휴지통은 하나의 순위를 나눠 쓴다.** 두 화면은 게임이 놓이는 자리만 다르고
순위는 같은 줄에서 나온다. 그래서 홈에서는 휴지통 게임의 번호가 비고, 빠진 번호는
휴지통 화면에 있다. 화면별로 다시 매기면 번호가 촘촘해지는 대신 같은 숫자가 서로
다른 뜻이 되어 통계가 갈라진다 — 휴지통에 넣은 게임이 나중에 살아날 수도 있으므로
갈라놓지 않는다.

카드는 `shared/GameCard.vue` 하나를 두 화면이 쓴다. 휴지통 게임이 3위 안으로
올라오면 그 자리에서 바로 메달이 붙어야 하는데, 카드가 둘이면 한쪽만 고치고
지나치기 쉽다.
