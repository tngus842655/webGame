...
휴지통 게임은 레지스트리에 있으므로 순위를 받는다 — 홈에서는 그 번호가 비고,
빠진 번호는 휴지통 화면에 있다(휴지통 카드도 같은 자리에 같은 숫자를 단다).
휴지통을 빼고 다시 매기면 번호가 촘촘해지는 대신 홈과 휴지통의 숫자가 서로 다른
뜻이 되어버려서, 하나의 순위를 나눠 쓰는 쪽을 택했다.
