...
목표는 30개다. 그 이상으로 늘어나면 인기 상위 30위 밖으로 밀린 게임을 덜어낸다.
바로 지우지 않고 휴지통에 `RETENTION_DAYS`(30일) 보관한 뒤 코드에서 제거한다.

- **자동으로 버리지 않는다.** 인기도는 오르내리는 값이라 잠깐 밀린 게임이 자동으로
  삭제 대기에 들어가면 곤란하고, 최종 삭제는 결국 사람이 하는 커밋이다.
  관리자 화면은 30위 밖 게임의 순위를 주황색으로 **표시만** 하고, 버리는 건 버튼으로 한다.
- 휴지통에 들어가는 순간 홈·랭킹 목록에서 빠진다(`sortByPopularity`가 거른다).
  `/play/<slug>` 직접 접근은 막지 않는다 — 북마크로 들어온 사람을 끊을 이유가 없다.
- 보관 기간이 지나면 화면에 '제거 가능'으로 뜬다. 그때 하는 일은 **커밋**이다:
  `src/games/<slug>/` 삭제 → `registry.ts`에서 제거 → 게임 제목 번역 키 정리.
  `game_flags` 행은 남겨둬도 무해하다(레지스트리에 없으면 화면에 안 나온다).
- 되돌리기는 기간 안이든 후든 언제든 된다 — 코드를 아직 안 지웠으면 그대로 살아난다.
